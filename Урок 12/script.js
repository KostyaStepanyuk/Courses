...
let formDef1 = undefined;
let formDef2 = undefined;

(function getFormDef(){
    function getNextFormDef(){
        if (formDef1 === undefined){
            $.ajax("https://fe.it-academy.by/Examples/dyn_form_ajax/formDef1.json",
                { type:'GET', dataType:'json', success:function(data) 
                { formDef1 = data; buildForm(formDef1); getNextFormDef(); }, 
                error:errorHandler }
            );
            return;
        }
        if (formDef2 === undefined){
            $.ajax("https://fe.it-academy.by/Examples/dyn_form_ajax/formDef2.json",
                { type:'GET', dataType:'json', success:function(data) 
                { formDef2 = data; buildForm(formDef2); getNextFormDef();}, 
                error:errorHandler }
            );
            return;
        }
